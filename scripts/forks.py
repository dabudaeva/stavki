...
class compute_bet():
    ...
    def best_combinations3(self):
        r_a = 1 / self.coefficients[0]
        r_b = 1 / self.coefficients[1]
        r_c = 1 / self.coefficients[2]
        l_a = math.ceil(self.deposit / self.coefficients[0] / (r_a+r_b+r_c))
        l_b = math.ceil(self.deposit / self.coefficients[1] / (r_a+r_b+r_c))
        l_c = math.ceil(self.deposit / self.coefficients[2] / (r_a+r_b+r_c))
        best_combination = [l_a, l_b, l_c]
        return best_combination